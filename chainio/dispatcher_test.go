...
func TestNotifyAndWaitOnConsumerTimeout(t *testing.T) {
	...
	consumer.On("ProcessBlock", mock.Anything).Return().Run(
...
func TestDispatchSequential(t *testing.T) {
	...
	// We are sending the blockbeat to three consumers, so we expect it to
	// be copied three times.
	mockBeat.On("copy").Return(mockBeat).Times(3)
...
func TestNotifyQueuesSuccess(t *testing.T) {
	...
	// We are sending the blockbeat to two consumers, so we expect it to be
	// copied two times.
	mockBeat.On("copy").Return(mockBeat).Twice()
	...
	// both calls.
	consumer1.On("ProcessBlock", mockBeat).Return(nil)
	consumer2.On("ProcessBlock", mockBeat).Return(nil)
...
func TestNotifyQueuesError(t *testing.T) {
	...
	// We are sending the blockbeat to one consumer, so we expect it to be
	// copied once.
	mockBeat.On("copy").Return(mockBeat).Once()
	...
	consumer.On("ProcessBlock", mockBeat).Return(errDummy)
