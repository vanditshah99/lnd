...
func (b *BlockbeatDispatcher) dispatchBlocks(
	...
	for {
		...
		case blockEpoch, ok := <-blockEpochs.Epochs:
			...
			if err != nil {
				clog.Criticalf("Notify block failed: %v", err)
			}

			clog.Infof("Notified all consumers on new block "+
...
func (b *BlockbeatDispatcher) notifyQueues() error {
	...
	for qid, consumers := range b.consumerQueues {
		clog.Debugf("Notifying queue=%d with %d consumers",
			qid, len(consumers))
		...
		go func(qid uint32, c []Consumer, beat Blockbeat) {
			...
			errChan <- beat.DispatchSequential(c)
	...
	for qid, errChan := range errChans {
		...
		case err := <-errChan:
			...
			clog.Debugf("Notified queue=%d", qid)
